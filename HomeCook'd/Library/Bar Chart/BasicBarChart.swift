...
class BasicBarChart: UIView {
    ...
    private let presenter = BasicBarChartPresenter(barWidth: 20, space: 20)
    ...
    private func showEntry(index: Int, entry: BasicBarEntry, animated: Bool, oldEntry: BasicBarEntry?) {
        ...
        mainLayer.addTextLayer(frame: entry.textValueFrame, color: UIColor.white.cgColor, fontSize: 10, text: entry.data.textValue, animated: animated, oldFrame: oldEntry?.textValueFrame)

        // Show a title below the bar
        mainLayer.addTextLayer(frame: entry.bottomTitleFrame, color: UIColor.lightGray.cgColor, fontSize: 10, text: entry.data.title, animated: animated, oldFrame: oldEntry?.bottomTitleFrame)
    ...
    private func showHorizontalLines() {
        ...
        lines.forEach { (line) in
            mainLayer.addLineLayer(lineSegment: line.segment, color: UIColor.white.cgColor, width: line.width, isDashed: line.isDashed as NSNumber, animated: false, oldSegment: nil)
        }
    }
}
